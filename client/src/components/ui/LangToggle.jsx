...
export function LangToggle({ compact = false }) {
  ...
  return (
    <div className="flex gap-1 bg-white/80 rounded-pill px-1 py-1 shadow-card backdrop-blur">
      {SUPPORTED.map((lang) => (
        <button
          key={lang.code}
          type="button"
          ...
          className={[
            ...
            current === lang.code ? 'bg-primary text-white' : 'text-neutral-900/70 hover:bg-primary-50',
          ].join(' ')}
